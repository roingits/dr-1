...
youdao.param_url = ["youdao_conv_url"];
...
youdao.urlInfo = function (c) {
    ...
    var u = youdao.param_url;
    if (typeof (youdao_conv_url) == "undefined" || youdao_conv_url == null) {
        var f = youdao.loc();
        a += youdao.concat("url", f + c);
    } else {
        var v = window[u[0]];
        a += youdao.concat("url", v + c);
    }
    return a
};
youdao.loc = function () {
...
youdao.cmImageRequest = function () {
    ...
    youdao.img.src = youdao.protocol() + "//dsp.youdao.com/cf.gif?source=" + a + "&r=" + (new Date()).getTime() + "&dgul=1559324";
